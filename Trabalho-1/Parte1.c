...
Cabecalho *Preencher_cabecalho(char string[]){
    Cabecalho *cabecalho;
...
void funcionalidade1(char nome_de_arquivo[]){
    ...
    dados = (Registro_de_Dados*) malloc (sizeof(Registro_de_Dados));
    Cabecalho *cab1;
    cab1 = (Cabecalho*) malloc (sizeof(Cabecalho));
    ////////////////////////////////////////////////////////////////////////
    int flag = 0,tam = 80;
...
void funcionalidade2(char nome_de_arquivo[]){
    ...
    FILE *arquivoTrab1si;
    arquivoTrab1si = fopen(nome_de_arquivo,"r+b");
...
se acinonado e da break*/
    while(1){
            ...
            if(!(feof(arquivoTrab1si) == 0)){
                break;
            }
            //Posicao
            fseek(arquivoTrab1si,-1,SEEK_CUR);
    }
    printf("Número de páginas de disco acessadas: %ld",(ftell(arquivoTrab1si)/16000));
    ...
    while(1){
            ...
            if(strcmp(nome_campo,"nroInscricao") == 0){
                ...
                if(inscricao == valor1){
                        ...
                        char data[10];
                        fread(&data,sizeof(char),10,arquivoTrab1si);
                        //tratamento de campo nulo da data
                        if(data[0] != '\0'){
                            printf(" ");
                            for(int i=0;i<10;i++){
                                printf("%c",data[i]);
                    ...
                    printf("\n");
                    //Calcular o numero de paginas pecorridas
                    registro = 1 + ftell(arquivoTrab1si)/16000;
                    printf("Número de páginas de disco acessadas: %d",registro);
                    entrou++;
                    return;
                }
            }
            ...
            registro++;
/////////////////////////////////////////////////////////////////////////////////////
/*Se algum campo dos que pode ter repeticoa forem encontrados entao a flag sinalizado
com 1 acionar o if que imprimir volta atras para o incio do registro e imprimir na
tela*/
            if(flag == 1){
                fseek(arquivoTrab1si,-80,SEEK_CUR);
                char removido;
                fread(&removido,sizeof(char),1,arquivoTrab1si);
                ////////////////////////////////////////////////
                int encadeamento;
                ...
                flag = 0;
                entrou++;
                aux_registro = registro;
            }
    }
    //Caso o nenhum registro tenha sido encontrado
    if(entrou == 0){
        printf("Registro inexistente.");
    }
    //Para os campos com possivel repeticao que devem pecorre o arquivo inteiro
    else if(strcmp(nome_campo,"nota") == 0){
        aux_regis = ftell(arquivoTrab1si)/16000;
        printf("Número de páginas de disco acessadas: %d",aux_regis);
        return;
    }
    else if(strcmp(nome_campo,"data") == 0){
        aux_regis = ftell(arquivoTrab1si)/16000;
        printf("Número de páginas de disco acessadas: %d",aux_regis);
        return;
    }
    else if(strcmp(nome_campo,"cidade") == 0){
        //Caso o fluxo seja arendondo para cima
        aux_regis = 1 + ftell(arquivoTrab1si)/16000;
        if(aux_regis == 27){
            aux_regis--;
        }
        printf("Número de páginas de disco acessadas: %d",aux_regis);
